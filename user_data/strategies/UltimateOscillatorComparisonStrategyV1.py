...
class UltimateOscillatorComparisonStrategyV1(IStrategy):
    """
    ...
    def populate_entry_trend(self, dataframe, metadata):
        dataframe.loc[
            (dataframe['uo'] > self.buy_uo.value) & (dataframe['uo'].shift(1) <= self.buy_uo.value),
            'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe, metadata):
        dataframe.loc[
            (dataframe['uo'] < self.sell_uo.value) & (dataframe['uo'].shift(1) >= self.sell_uo.value),
            'exit_long'] = 1
        return dataframe
