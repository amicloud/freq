...
class SimpleRL3(BaseReinforcementLearningModel):
    """
    ...
    class MyRLEnv(Base3ActionRLEnv):  # type: ignore[no-redef]
        def calculate_reward(self, action: int) -> float:
            ...
            #logger.info(f"Current tick: {self._current_tick}")
            # This should only be true at the start of an episode
            ...
            if ((action == Actions.Neutral.value) or (action == Actions.Buy.value)) and self._position == Positions.Long:
                ...
                if pnl < 0:
                    if pnl_change < 0:
                        if trade_duration < patience:
                            reward =  -0.001
                        else:
                            reward = -(1 + abs(pnl_change * 10) * (1.0 + abs(pnl)))
                    if pnl_change > 0:
                        reward =  (1 + pnl_change) * 5
                elif pnl > 0:
                    if pnl_change > 0:
                        reward =  ((1 + pnl_change) * (1))  * 10
                    if pnl_change < 0:
                        reward = -(1 + abs(pnl_change)) * 10
