...
class RL5(BaseReinforcementLearningModel):
    """
    ...
    class MyRLEnv(Base3ActionRLEnv):  # type: ignore[no-redef]
        def calculate_reward(self, action: int) -> float:
            ...
            if ((action == Actions.Neutral.value) or (action == Actions.Buy.value)) and self._position == Positions.Long:
                reward = (pnl/100) + (pnl_change * 10)
                #reward += pnl/10
                reward += (entropy * (trade_duration/max_trade_duration))
                if False:
                    logger.info("----------")
                    logger.info(f"PnL: {pnl}")
                    logger.info(f"∆:   {pnl_change}")
                    logger.info(f"Td:  {trade_duration}")
                    logger.info(f"RW:  {reward}")
                    logger.info("----------")

            if (action == Actions.Neutral.value or action == Actions.Sell.value) and self._position == Positions.Neutral:
                reward = entropy
            ...
            if action == Actions.Sell.value and self._position == Positions.Long:
                if pnl > 0:
                    ...
                    reward = -0.01
                #logger.info(f"Sold for reward: {reward}")

            alpha = 2
            j = (self._total_profit - 1)/10

            if self._total_profit > 1:
                reward += j #(self._total_profit - 1)/10
            elif (self._total_profit < 1) and self._total_profit > 0.4:
                reward += j 
            else:
                reward += 0

            return tanh(alpha * reward)
